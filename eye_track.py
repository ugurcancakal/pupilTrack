...
def main(filepath, params):
    ...
    while True:
        ...
        region = initial_region_estimation(gray_roi,radius=32, step=16, h_factor=2, trim=False, outer=False)

        # cv2.imshow("ROI", to_gray(roi))
...
def initial_region_estimation(frame, radius, step, h_factor=1.5, trim=True, outer=False):
    '''
    ...
    to the elliptical pupil shape.
        ...
        Arguments:
            ...
            trim(bool):
                flag for trim or highlight
                Default=True

            outer(bool):
                draw the outer rectangle or not
    ...
    region = get_roi(frame, center, haar_radius, trim, outer)

    return region

def conv_int(frame_int, kernel, step, padding):
    '''
    Convolution on the integral image.
    Requires integral frame
...
def get_roi(frame, center, radius, trim=False, outer=False, color=(0, 255, 0), thickness=2):
    '''
    Region of Interest given the frame defined by 
    ...
    if trim:
        y = (start[1], end[1])
        x = (start[0], end[0])
        frame = clip(frame, y, x)
    else:
        cv2.rectangle(frame, start, end, color, thickness)
        if outer:
            start_big = tuple([val-3*radius for val in center])
            end_big = tuple([val+3*radius for val in center])
            cv2.rectangle(frame, start_big, end_big, color, thickness)
