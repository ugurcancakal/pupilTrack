...
class EyeTrack(unittest.TestCase):
	...
	# def test_eye_detect(self):
	# 	'''
	# 	'''
	# 	# Load Image
	# 	# filepath = os.path.join(test_input, eye_img)
	# 	for i in range(500,900):
	# 		filepath = os.path.join(data_dir, 'p1-left', 'frames', f'{i}-eye.png')
	# 		frame = cv2.imread(filepath)

	# 		region, response, _ = eye.detect_pupil(frame, radius=72, step=8, h_factor=1.4)
	# 		roi_pupil = eye.clip_rect(eye.to_gray(frame), region)

	# 		region_iris = eye.rect_enlarge(region, 1.6)
	# 		roi_iris = eye.clip_rect(eye.to_gray(frame), region_iris)

	# 		roi_pupil = eye.clustered_frame(roi_pupil, 5)
	# 		roi_iris = eye.clustered_frame(roi_iris, 5)

	# 		threshold_pupil, threshold_iris = eye.get_threshold(roi_iris, 5)
	# 		print(threshold_pupil, threshold_iris)
	# 		_,t_roi_pupil = cv2.threshold(roi_pupil, threshold_pupil, 255, cv2.THRESH_BINARY_INV)
	# 		_,t_roi_iris = cv2.threshold(roi_iris, threshold_iris, 255, cv2.THRESH_BINARY_INV)

	# 		t_roi_pupil = cv2.blur(t_roi_pupil,(5,5))
	# 		t_roi_iris = cv2.blur(t_roi_iris,(5,5))

	# 		r_contours = eye.get_contours(roi_pupil, threshold_pupil)
	# 		i_contours = eye.get_contours(roi_iris, threshold_iris)

	# 		print(len(r_contours))

	# 		print(len(i_contours))

	# 		# pupil = None
	# 		# th_pupil, th_iris = eye.get_threshold(roi)
	# 		# print(th_pupil, th_iris)
	# 		# contours = eye.get_contours(roi, th_pupil)
	# 		# if len(contours):
	# 		# 	pupil = eye.fit_ellipse_LSQ(contours[0])

	# 		# contours = eye.get_contours(roi, th_iris)
	# 		# if len(contours):
	# 		# 	for contour in contours:
	# 		# 		print(cv2.contourArea(contour))
	# 		# 		iris = eye.fit_ellipse_LSQ(contour)
	# 		# 		frame = eye.overlay_pupil(frame, iris, offset=region[0])
	# 		# 		cv2.imshow("grayscale", frame)
	# 		# 		key = cv2.waitKey(0)

	# 		frame = eye.overlay_rectangle(frame, region)
	# 		frame = eye.overlay_rectangle(frame, region_iris)
	# 		# # frame = eye.overlay_pupil(frame, pupil, offset=region[0])
			
	# 		# pupil,iris = eye.segment_eye(roi)
	# 		# if pupil is not None:
	# 		# 	frame = eye.overlay_ellipse(frame, pupil, offset=region[0])
	# 		# if iris is not None:
	# 		# 	frame = eye.overlay_ellipse(frame, iris, offset=region[0], center=False)

	# 		# frame_gray = eye.to_gray(frame)
	...
	# 		cv2.imshow("grayscale", frame)
	# 		cv2.imshow("roi_pupil", roi_pupil)
	# 		cv2.imshow("roi_iris", roi_iris)
	# 		cv2.imshow("t_roi_pupil", t_roi_pupil)
	# 		cv2.imshow("t_roi_iris", t_roi_iris)
	# 		key = cv2.waitKey(0)
	# 		break

	# def test_eye_detect(self):
	# 	in_file = os.path.join(test_input, eye_vid)
	# 	out_file = os.path.join(test_output, 'pupil.avi',)
	# 	video_pupil, fps = eye.process_video(in_file)
	...
	# 		# 1 Find pupil 
	# 		pupil_region,response,radius = eye.radius_search(frame,(32,96),4)
	# 		pupil_roi = eye.clip_rect(frame,pupil_region)
	# 		th = eye.get_threshold(pupil_roi)
	# 		th_pupil = th[0]
	# 		# print(th)
	# 		pupil = eye.get_pupiris(pupil_roi, th_pupil)
			
	# 		# 2 Find Iris
	# 		iris_region = eye.rect_enlarge(pupil_region, 1.8)
	# 		iris_roi = eye.clip_rect(frame,iris_region)
	# 		th = eye.get_threshold(iris_roi)
	# 		th_iris = th[1]
	# 		# print(th)
	# 		iris = eye.get_pupiris(iris_roi, th_iris)

	# 		# Correct iris
	# 		pupil= eye.ellipse_offset(pupil,pupil_region[0])
	# 		iris= eye.ellipse_offset(iris,iris_region[0])
	# 		iris = (pupil[0], iris[1], pupil[2])

	# 		# print(iris)
	# 		# print(pupil)
	# 		status = {}
	# 		status['center'] = pupil[0]
	# 		status['theta'] = pupil[2]
	# 		status['iris_ax'] = pupil[1]
	# 		status['pupil_ax'] = iris[1]
	# 		status['haar_radius'] = radius
	# 		status['pupil_region'] = pupil_region
	# 		status['iris_region'] = iris_region
	# 		print(type(radius))

	# 		frame = eye.overlay_rectangle(frame, pupil_region)
	# 		frame = eye.overlay_rectangle(frame, iris_region)
	# 		frame = eye.overlay_ellipse(frame, pupil)
	# 		frame = eye.overlay_ellipse(frame, iris, center=False)
	# 		frame = eye.overlay_status(frame, status)

	# 		# cv2.imshow("pupil_roi", eye.clustered_frame(pupil_roi,5))
	# 		# cv2.imshow("iris_roi", eye.clustered_frame(iris_roi,5))
	# 		cv2.imshow("frame", frame)
	# 		key = cv2.waitKey(0)
	...
	# 		eye_region = eye.rect_enlarge(pupil_region, 2)

	# 		pupil_roi = eye.clip_rect(frame,pupil_region)
	# 		eye_roi = eye.clip_rect(frame,eye_region)

	# 		th = eye.get_threshold(eye_roi)
	# 		th_pupil = th[0]
	# 		th_iris = th[1]
	# 		# print(th)

	# 		# 2 Find Iris and Pupil
	# 		pupil = eye.get_pupiris(pupil_roi, th_pupil)
	# 		iris = eye.get_pupiris(eye_roi, th_iris)			

	# 		# Correct iris
	# 		pupil= eye.ellipse_offset(pupil,pupil_region[0])
	# 		iris= eye.ellipse_offset(iris,eye_region[0])
	# 		iris = (pupil[0], iris[1], pupil[2])
	...
	# 		status['haar_radius'] = radius
	# 		status['pupil_region'] = pupil_region
	# 		status['eye_region'] = eye_region

	# 		frame = eye.overlay_rectangle(frame, pupil_region)
	# 		frame = eye.overlay_rectangle(frame, eye_region)
	# 		frame = eye.overlay_ellipse(frame, pupil)
	# 		frame = eye.overlay_ellipse(frame, iris, center=False)
	# 		frame = eye.overlay_status(frame, status)
	...
	# def test_get_pupiris(self):
	# 	in_file = os.path.join(test_input, eye_vid)
	# 	out_file = os.path.join(test_output, 'submission.avi')
	...
	# 	# pof = eye.PupirisOnFrame(update_limit=(0,5), radius_range=(32,96), r_step=8, xy_step=8, h_factor=1, iris=False)
	...
	def test_pupiris(self):
		pof = eye.PupirisOnFrame(update_limit=(1,10), radius_range=(56,64), r_step=4, xy_step=8, np_clusters=2, ni_clusters=3, h_factor=1.2, iris=True)
		for i in range(150,940):
			print(f'Frame {i}')
			filepath = os.path.join(data_dir, 'p2-left', 'frames', f'{i}-eye.png')
			frame = eye.process_image(filepath, pof)
			cv2.imshow("frame", frame)
			key = cv2.waitKey(0)

		# eye.save_video(frames, 'test_equality.avi', 24)
